...
import { AccountReportItemProvider, AccountReportItemContext, ROWS_PER_PAGE } from "./AccountReportItemProvider"
import { ResolveModalRef, ResolveModalRefSelectors } from "./ResolveModal"
import { ClipboardPenLine } from "lucide-react"

const WrappedAccountReportItem = () => {
    const { swrs, reducer } = useContext(AccountReportItemContext)!
    const { accountReportsSwr } = swrs
    const { data: reportData, isLoading } = accountReportsSwr
    ...
    const renderCell = useCallback((report: ReportAccountEntity, columnKey: React.Key) => {
        switch (columnKey) {
        case "reporter":
    ...
    const handleResolve = (report: ReportAccountEntity) => {
        ...
        <div>
            ...
            >
                ...
                <TableBody
                    ...
                    emptyContent="No account reports yet."
                >
                    {(item) => (
                        <TableRow key={item.reportAccountId}>
                            {
...
export const AccountReportItem = () => {
    return (
        <AccountReportItemProvider>
            <WrappedAccountReportItem />
        </AccountReportItemProvider>
    )
}