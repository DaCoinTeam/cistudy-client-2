...
const Page = () => {
    ...
    const handleDelete = async (id?: string) => {
        let data = []
        if (id) data = [id]
        else data = [...selectedItems]
        const res = await deleteFromCart({
            data: { cartCourseIds: data },
        })
        if (res.others) {
            await mutate()
            if (id) dispatch({ type: "SET_UNSELECT_ITEM", payload: id })
            else {
                dispatch({
                    type: "SET_UNSELECT_ALL_ITEMS",
                })
                ...
                <div className='mx-auto col-span-5  w-full'>
                  ...
                  All Products
                            ...
                            <div className='flex items-end mr-4'>
                                <Link
                                    isDisabled={cartCourses?.length === 0}
                                    ...
                                    >
                                        ...
                                        <div className='w-full'>
                                            <CartItem
                                                ...
                                                cartCourseId={item.cartCourseId}
                                            />
                                        </div>
                                    </div>
                                ))
